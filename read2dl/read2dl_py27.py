...
class read2dl:
    ...
    def get_record(self, index):
        ...
        record = str('')
        record_size = ord(self.fileContent[index])
        index += 1
        if record_size == 130:
            print("end of file")
            return
...
if __name__ == "__main__":
    #EClib = read2dl("/home/eric/cernbox/FDD_libraries/ue488g34.2dl") #89Sr [0001]
    ...
    for i in range(0,2):
        plt.figure(figN)
        figN += 1
        imgmat = np.array(ECdict["Spectrums"][i]["array"]).reshape((ny, nx))
        plt.contourf(imgmat)
    plt.show()
